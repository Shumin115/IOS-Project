...
class MasterViewController: UITableViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            deleteOnline(item: self.objects[indexPath.row]) { (success) in print(success) }
    ...
    func deleteOnline(item: PicogramItem, completion: @escaping (Bool)->Void) {
        ...
        asClient?.perform(mutation: mut) { (result, error) in
       completion(error == nil)
        }
    }
    
    func fetchItem(completion: @escaping (Bool)->Void) {
        let q = ListPicosQuery()
        let filter = ModelStringInput(eq: AWSMobileClient.default().username)
        let filterInput = ModelPicoFilterInput(userName: filter)
        q.filter = filterInput
        asClient?.fetch(query: q, cachePolicy: .fetchIgnoringCacheData, resultHandler: { (result, error) in
            guard error == nil else { completion(false); return}
            guard let posts = result?.data?.listPicos?.items else {
            ...
            posts.forEach { (item) in
                let np = PicogramItem()
                np.id = item?.id ?? "0"
                np.userName = item?.userName ?? "noname"
                np.imageName = item?.imageName
